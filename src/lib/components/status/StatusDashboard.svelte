...
<div class="status-shell min-h-screen bg-white text-gray-900 dark:bg-gray-950 dark:text-gray-100">
	<header
		class="sticky top-0 z-20 flex items-center justify-between gap-3 border-b border-gray-200 bg-white/90 px-2 py-3 backdrop-blur dark:border-gray-850 dark:bg-gray-950/90 sm:px-3"
	>
		<div class="min-w-0">
	...
	{:else if error}
		<div class="px-2 py-8 text-sm text-red-600 dark:text-red-300 sm:px-3">{error}</div>
	{:else if data}
		<main class="status-columns" style={`--status-columns: ${columnCount}`}>
							...
							{:else if moduleId === 'daily-tokens'}
								<div class="p-3 sm:p-4">
									...
									<div class="mt-3 flex gap-4 text-xs text-gray-500">
										<span
											><i class="mr-1 inline-block h-2 w-2 rounded-sm bg-indigo-500"></i>Input</span
										>
										<span
											><i class="mr-1 inline-block h-2 w-2 rounded-sm bg-cyan-500"></i>Output</span
										>
									</div>
								</div>
